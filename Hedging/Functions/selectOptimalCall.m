function [cashFlow, idxCall, quantityCall, spesaBidAsk] = selectOptimalCall(EtaUpSensitivity, CallexpDates, timegrid, askCall, bidCall, ValueChangeCall, flag, idx)
    ...
    % 1.5)
    if flag == 1
        maskMaturity = zeros(length(CallexpDates), 1);
        maskMaturity(idx) = 1;
    end
    ...
    % indici candidati
    maskValid = maskMaturity & maskNonZero  & maskSpread;
