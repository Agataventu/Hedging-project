function ValueChangeCall= etaSensitivitycall(alpha, CallexpDates, CallPrices, disc, dates, CallStrikes, fwd_prices, eta, kappa, sigma, eta_omega, kappa_omega, sigma_omega, t0)
...
for i = 1:length(CallexpDates)
    ...
    %if ~isnan(CallPrices(i)) && ~(mon < 0.005 && mon > -0.005) && year(CallexpDates(i))~=2017 && year(CallexpDates(i))~=2018
        ...
        pricesModelEtaUp =[ pricesModelEtaUp, PriceCallOption(CallStrikes(i),fwd_prices(idx),disc_i,@(u)phi_fun(u,ttm_i,eta_omega, kappa_omega, sigma_omega),M,dz,a)];
