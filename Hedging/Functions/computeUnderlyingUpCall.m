function valueChange = computeUnderlyingUpCall( ...
...
for i = 1:length(CallexpDates)
    ...
    %if ~isnan(CallPrices(i)) && ~(mon < 0.005 && mon > -0.005) && year(CallexpDates(i))~=2017 && year(CallexpDates(i))~=2018
        ...
        pricesModelEtaUp =[ pricesModelEtaUp, PriceCallOption(CallStrikes(i),fwd_prices_up(idx), disc_i,@(u)phi_fun(u,ttm_i,eta, kappa, sigma),M,dz,a)];
