function valueChange = computeUnderlyingUpPut( ...
    PutStrikes, PutexpDates, dates, ...
    fwd_prices, disc, ...
    alpha, eta, kappa, sigma, ...
    t0)
fwd_prices_up = (fwd_prices ./ disc(2:end) + 1) .*disc(2:end);
if alpha == 0
...
for i = 1:length(PutexpDates)
    ...
    %if ~isnan(CallPrices(i)) && ~(mon < 0.005 && mon > -0.005) && year(CallexpDates(i))~=2017 && year(CallexpDates(i))~=2018
        ...
        pricesModelEtaUp = [pricesModelEtaUp, PriceCallOption(PutStrikes(i),fwd_prices_up(idx),disc_i,@(u)phi_fun(u,ttm_i,eta, kappa, sigma),M,dz,a)-disc_i * (fwd_prices(idx) - PutStrikes(i))];
...
valueChange=pricesModelEtaUp-pricesModel;